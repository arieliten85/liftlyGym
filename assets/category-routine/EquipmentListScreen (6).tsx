...
// Calcula cuántos equipos le corresponden a cada grupo muscular dentro del cap.
// Lógica: base equitativo para todos, los grupos grandes se llevan el resto primero.
...
export default function EquipmentListScreen() {
  ...
  // Límite por grupo muscular calculado según nivel y prioridad de músculo grande.
  // Se usa para deshabilitar cards cuando se alcanzó el cupo del grupo.
  const slotsPerMuscle = useMemo(
    () => calcSlotsPerMuscle(muscleGroups, experience),
            ...
            <View key={section.muscleId} style={styles.sectionWrapper}>
              ...
              >
                {section.items.map((item, itemIndex) => {
                  ...
                  const muscleLimit = slotsPerMuscle[section.muscleId] ?? 99;
                  const muscleSelected = section.items.filter((i) =>
                    selectedItems.includes(i.id),
                  ).length;
                  // Deshabilitar si: no está seleccionado Y ya se alcanzó el límite del grupo
                  const isDisabled = !isSelected && muscleSelected >= muscleLimit;
                  return (
                    <TouchableOpacity
                      key={item.id}
                      style={[
                      ...
                      onPress={() => !isDisabled && toggleEquipmentItem(item.id)}
