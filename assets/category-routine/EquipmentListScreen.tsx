...
// ─── Pool de equipment por músculo según nivel ────────────────────────────────
//
// La IA necesita suficiente variedad para elegir, pero no todo el catálogo.
// Se envía un pool con margen de +1 sobre los ejercicios esperados por músculo:
//
//   Principiante → 3 ejercicios/músculo → pool de 4 items/músculo
//   Intermedio   → 4 ejercicios/músculo → pool de 5 items/músculo
//   Avanzado     → 5 ejercicios/músculo → pool de 6 items/músculo (completo)
//
// Los items en equipmentList.data.ts ya están ordenados de más a menos
// fundamental, así que los primeros N son siempre los más relevantes.
function equipmentPoolByLevel(experience: string | null): number {
  switch (experience) {
    case "avanzado":    return 6; // pool completo — cubre los 5 ejercicios + margen
    case "intermedio":  return 5; // pool amplio   — cubre los 4 ejercicios + margen
    case "principiante":
    default:            return 4; // pool base     — cubre los 3 ejercicios + margen
  }
}

// ─── Resolver automático — pool ajustado por nivel ───────────────────────────
function resolveAllEquipmentIds(
  ...
  const poolLimit = equipmentPoolByLevel(experience);
  ...
  for (const muscle of muscleGroups) {
    ...
    for (const item of items) {
      if (addedForMuscle >= poolLimit) break;
      if (!seen.has(item.id)) {
        seen.add(item.id);
        result.push(item.id);
        addedForMuscle++;
...
export default function EquipmentListScreen() {
    ...
    (equipmentItems: string[], skipped: boolean) => {
      const resolvedEquipment =
        equipmentItems.length > 0
          ? equipmentItems
          : resolveAllEquipmentIds(equipment, muscleGroups, experience); // ← experience pasado
      ...
      const payload = {
        ...
        selectedEquipmentItems: {
          mode:
            skipped || equipmentItems.length === 0
              ? `auto — pool de ${equipmentPoolByLevel(experience)} equipos/músculo (nivel: ${experience ?? "principiante"})`
        ...
        routineClassification: {
          ...
          exercisesPerMuscle: classification.exercisesPerMuscle, // 3 / 4 / 5
          setsRange: classification.setsRange,                   // "3 series · 12-15 reps"
          levelSummary: classification.levelSummary,
        },
      };
  ...
  const borderDef = isDark ? "rgba(46,207,190,0.15)" : theme.colors.border; // ← typo corregido: "rgbaj" → "rgba"
