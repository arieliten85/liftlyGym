...
// Cupo total fijo de equipos (= ejercicios) por nivel.
// El back construye la rutina solo con estos ids — el total YA es el límite.
//   principiante → 4: solo movimientos base, foco en aprender técnica
//   intermedio   → 6: base + un accesorio por grupo
//   avanzado     → 8: mayor variedad e intensidad
const TOTAL_EQUIPMENT_CAP: Record<string, number> = {
  principiante: 4,
  intermedio: 6,
  avanzado: 8,
};

// Prioridad de músculos: 1 = grande/compuesto, 2 = mediano, 3 = accesorio.
// Los grandes reciben el doble de slots cuando el cupo se distribuye.
...
function resolveAllEquipmentIds(
  ...
  const totalCap = TOTAL_EQUIPMENT_CAP[experience ?? "principiante"] ?? 4;

  // Ordenar: músculos grandes primero
  const sorted = [...muscleGroups].sort(
    (a, b) => (MUSCLE_PRIORITY[a] ?? 2) - (MUSCLE_PRIORITY[b] ?? 2),
  );

  // Pesos: grande=2, mediano=1.5, accesorio=1
  const weightOf = (m: string) => {
    const p = MUSCLE_PRIORITY[m] ?? 2;
    if (p === 1) return 2;
    if (p === 2) return 1.5;
    return 1;
  };
  ...
  // Slots proporcionales, mínimo 1 por grupo
  const slots = weights.map((w) =>
    Math.max(1, Math.floor((w / totalWeight) * totalCap)),
  );

  // Ajustar suma exacta al cap (agregar al grupo de mayor peso primero)
  let assigned = slots.reduce((s, n) => s + n, 0);
  let i = 0;
  while (assigned < totalCap) {
    slots[i % sorted.length]++;
    assigned++;
    i++;
  }
  while (assigned > totalCap) {
    const idx = (slots.length - 1) - (i % slots.length);
    if (slots[idx] > 1) { slots[idx]--; assigned--; }
    i++;
  }

  // Recolectar ids tomando los primeros N de cada grupo (orden = más compuesto primero)
  const seen = new Set<string>();
  const result: string[] = [];
  sorted.forEach((muscle, idx) => {
    const items = getEquipmentList(equipment, muscle);
    let count = 0;
    for (const item of items) {
      if (count >= slots[idx]) break;
      if (!seen.has(item.id)) {
        seen.add(item.id);
        result.push(item.id);
        count++;
...
// Coincide con el cap — mismo número que se manda al back como límite explícito.
function resolveMaxExercises(experience: string | null): number {
  return TOTAL_EQUIPMENT_CAP[experience ?? "principiante"] ?? 4;
}
